...
import type { EditOrderFormValues } from '../hooks/use-edit-order';
...
export function EditOrderForm({
  ...
  const renderProductList = (items: Product[], title: string) => {
    if (items.length === 0) {
      return (
        <div className="text-center text-xs p-4 text-text-secondary bg-white rounded-3xl border border-dashed border-gray-200 flex flex-col items-center justify-center gap-2">
          {ORDERS_CONSTANTS.form.noProducts}
          <button className="text-primary underline" onClick={() => navigate('/dashboard/products')}>
            Ir para o cadastro de produto
          </button>
        </div>
      );
    }
